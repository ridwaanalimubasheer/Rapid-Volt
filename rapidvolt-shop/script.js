...
function updateCartUI(){
  ...
  if(cartItemsEl){
    ...
    cart.forEach(i=>{
      ...
      row.innerHTML = `<div class="left"><strong>${i.title}</strong><div>AED ${i.price.toFixed(2)} x ${i.qty}</div></div><div class="right"><button class="qty" data-id="${i.id}" data-delta="-1">-</button><button class="qty" data-id="${i.id}" data-delta="1">+</button><button class="remove" data-id="${i.id}">Remove</button></div>`;
...
if(productsEl){
  PRODUCTS.forEach(p=>{
    ...
    card.innerHTML = `<h4>${p.title}</h4><p class='desc'>${p.desc}</p><div class='price'>AED ${p.price.toFixed(2)}</div><div class='actions'><button data-id='${p.id}' class='btn add-to-cart'>Add to cart</button></div>`;
...
if(checkoutForm && window.emailjs){
  ...
  checkoutForm.addEventListener('submit', function(e){
    ...
    const statusEl = document.getElementById('checkoutStatus'); if(statusEl) statusEl.textContent = 'Sending order...';
...
if(chatbot){
  ...
  function resetInactivityTimer() {
    ...
    inactivityTimer = setTimeout(()=>{
      ...
      chatBody.innerHTML='';
    }, 7 * 60 * 1000); // 7 minutes (adjust between 5-10 min)
  }
